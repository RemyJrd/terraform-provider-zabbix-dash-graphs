...
func resourceZabbixDashboardCreate(d *schema.ResourceData, meta interface{}) error {
	...
	dashboards := zabbix.Dashboards{*dashboard}
...
func resourceZabbixDashboardRead(d *schema.ResourceData, meta interface{}) error {
	...
	for i, page := range dashboard.Pages {
		...
		widgets, err := getPageWidgets(api, dashboard.DashboardID, page.PageID)
		if err != nil {
			return err
		}
...
func buildDashboardObject(d *schema.ResourceData, api *zabbix.API) *zabbix.Dashboard {
	...
	dashboardPages := make(zabbix.DashboardPages, len(pages))

	for i, p := range pages {
		page := p.(map[string]interface{})
		
		dashboardPage := zabbix.DashboardPage{
		...
		if widgets, ok := page["widget"].([]interface{}); ok && len(widgets) > 0 {
			dashboardWidgets := make(zabbix.DashboardWidgets, len(widgets))
			
			for j, w := range widgets {
				widget := w.(map[string]interface{})
				
				dashboardWidget := zabbix.DashboardWidget{
					...
					Fields: zabbix.DashboardWidgetFields{},
				}
...
func getPageWidgets(api *zabbix.API, dashboardID, pageID string) (zabbix.DashboardWidgets, error) {
	// For Zabbix API, we need to get the widgets from the dashboard
	dashboards, err := api.DashboardsGet(zabbix.Params{
	...
	if len(dashboards) != 1 {
		return nil, fmt.Errorf("Expected one dashboard with ID %s", dashboardID)
	}
	
	// Find the page and its widgets
	for _, page := range dashboards[0].Pages {
		if page.PageID == pageID {
			return page.Widgets, nil
		}
	}
	
	return zabbix.DashboardWidgets{}, nil
}